...
{
	face_recognize_out_t *face_recognize_out = NULL;
	...
	modelType = FACE_RECOGNIZE;
	outconfig.format = AML_OUTDATA_FLOAT32;
	outdata = (nn_output*)aml_module_output_get(qcontext,outconfig);
	face_recognize_out = (face_recognize_out_t*)post_process_facereg(outdata);

	if (face_recognize_out == NULL)
	{
	...
	for (i = 0; i < 512; i++)
	{
		printf("buffer[%d] = %f \n", i, face_recognize_out->faceVector[i]);
	}
