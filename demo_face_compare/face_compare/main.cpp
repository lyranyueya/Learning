...
{
	face_compare_out_t *face_compare_out = NULL;
	...
	else
	{
		...
		if (File == NULL)
		{
			printf("File null returned\n");
			return -1;
		}

		face_compare_data = (unsigned char *)malloc(160 * 160 * 3 * 2 * sizeof(char));
		fread(face_compare_data, sizeof(char),160 * 160 * 3 *2 ,File);
		fclose(File);
		if (face_compare_data == NULL)
		{
			printf("face_compare_data null returned\n");
			return -1;
		}
		inData.input = face_compare_data;
	...
	modelType = FACE_COMPARISION;
	outconfig.format = AML_OUTDATA_FLOAT32;
	outdata = (nn_output*)aml_module_output_get(qcontext,outconfig);
	face_compare_out = (face_compare_out_t*)post_process_face_compare(outdata);

	if (face_compare_out == NULL)
	{
		printf("aml_module_output_get error\n");
		return -1;
	}
	printf("compareScore = %f\n", face_compare_out->compareScore);
...
void* init_network(int argc,char **argv)
{
	...
	printf("the input type should be 160*160*3*2\n");
	input_width = 160;
	input_high = 160;
