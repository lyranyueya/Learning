...
{
	face_landmark5_out_t *face_landmark5_out = NULL;
	...
	modelType = FACE_LANDMARK_5;
	outconfig.format = AML_OUTDATA_FLOAT32;
	outdata = (nn_output*)aml_module_output_get(qcontext,outconfig);
	face_landmark5_out = (face_landmark5_out_t *)post_process_faceland5(outdata);
	if (face_landmark5_out == NULL)
	{
		printf("aml_module_output_get error\n");
		return -1;
	}
	for (i = 0; i < face_landmark5_out->detNum; i++)
	{
		printf("face_number:%d : box.x = %f , box.y = %f, box.w = %f, box.h = %f \n", i, face_landmark5_out->facebox[i].x, face_landmark5_out->facebox[i].y, face_landmark5_out->facebox[i].w, face_landmark5_out->facebox[i].h);
		printf("face_number:%d , left_eye.x = %f , left_eye.y = %f \n",        i ,face_landmark5_out->pos[i][0].x, face_landmark5_out->pos[i][0].y);
		printf("face_number:%d , right_eye.x = %f , right_eye.y = %f \n",      i ,face_landmark5_out->pos[i][1].x, face_landmark5_out->pos[i][1].y);
		printf("face_number:%d , nose.x = %f , nose.y = %f \n",                i ,face_landmark5_out->pos[i][2].x, face_landmark5_out->pos[i][2].y);
		printf("face_number:%d , left_mouth.x = %f , left_mouth.y = %f \n",    i ,face_landmark5_out->pos[i][3].x, face_landmark5_out->pos[i][3].y);
		printf("face_number:%d , right_mouth.x = %f , right_mouth.y = %f\n\n", i ,face_landmark5_out->pos[i][4].x, face_landmark5_out->pos[i][4].y);
	}
	return ret;
}
