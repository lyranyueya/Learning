...
int run_network(void *qcontext, unsigned char *qrawdata,int fbmode,unsigned char *fbbuf)
{
    ...
    cv::Mat temp;
    cv::Mat mask_rgb;
    unsigned char *half_raw = NULL;
    unsigned char *half_out = NULL;
	#endif
	...
	body_out = (body_pose_out_t*)postprocess_bodypose(outdata);
	if (body_out == NULL)
	{
		printf("aml_module_output_get bodypose error\n");
		return -1;
	}
	for (i = 0; i < 18;i++)
	{
		if (body_out->bpos[i].valid)
		{
			cv::circle(img,cv::Point((int)body_out->bpos[i].pos.x,(int)body_out->bpos[i].pos.y),5,cv::Scalar(0,255,0),-1,cv::LINE_AA);
			printf("i = %d,detect x = %f, detect y = %f\n", i,(float)body_out->bpos[i].pos.x, (float)body_out->bpos[i].pos.y);
		}
	}
	cv::imwrite("out_body_point.bmp",img);
	#else
	printf("Error:BODY_POSE depend on opencv environment");
	#endif
