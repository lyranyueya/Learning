...
int run_network(void *qcontext, unsigned char *qrawdata,int fbmode,unsigned char *fbbuf)
{
	...
	obj_detect_out = (obj_detect_out_t*)post_process_yolov3(outdata);

	if (obj_detect_out == NULL)
	{
...

	{
		...
		for (i =0;i < obj_detect_out->detNum;i++)
		{
			...
			left  = (obj_detect_out->pBox[i].x-obj_detect_out->pBox[i].w/2.)*cols;
			right = (obj_detect_out->pBox[i].x+obj_detect_out->pBox[i].w/2.)*cols;
			top   = (obj_detect_out->pBox[i].y-obj_detect_out->pBox[i].h/2.)*rows;
			bot   = (obj_detect_out->pBox[i].y+obj_detect_out->pBox[i].h/2.)*rows;
			if (left < 2) left = 2;
