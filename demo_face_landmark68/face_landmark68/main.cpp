...
{
	face_landmark68_out_t *face_landmark68_out = NULL;
	...
	else
	{
		for(i=0;i<60*60;i++)
		{
			data_land68[i] = qrawdata[3*i];
		}
		inData.input = data_land68;
		...
		modelType = FACE_LANDMARK_68;
		outconfig.format = AML_OUTDATA_FLOAT32;
		outdata = (nn_output*)aml_module_output_get(qcontext,outconfig);
		face_landmark68_out = (face_landmark68_out_t*)post_process_faceland68(outdata);
		if (face_landmark68_out == NULL)
		{
			printf("aml_module_output_get error\n");
			return -1;
		}
		for (i = 0; i < face_landmark68_out->detNum; i++)
		{
			for (j = 0; j < 68; j++)
			{
				printf("pos[%d][%d] x = %f , y = %f\n", i,j,face_landmark68_out->pos[i][j].x ,face_landmark68_out->pos[i][j].y);
			}
...
void* init_network(int argc,char **argv)
{
	...
	printf("the input type should be 60*60*1\n");
	input_width = 60;
	input_high = 60;
