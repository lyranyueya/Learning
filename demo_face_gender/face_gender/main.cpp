...
{
	face_gender_out_t *gender_out = NULL;
	...
	modelType = FACE_GENDER;
	outconfig.format = AML_OUTDATA_FLOAT32;
	outdata = (nn_output*)aml_module_output_get(qcontext,outconfig);
	gender_out = (face_gender_out_t*)post_process_face_gender(outdata);
	if (gender_out == NULL)
	{
		printf("aml_module_output_get error\n");
		return -1;
	}
	printf("The rate of femal : %f\n",gender_out->gender);
	if (gender_out->gender < 0.5)
		printf("This is male\n");
	else
...
void* init_network(int argc,char **argv)
{
	...
	printf("the input type should be 48*48*3\n");
	input_width = 48;
	input_high = 48;
