...
int run_network(void *qcontext, unsigned char *qrawdata,int fbmode,unsigned char *fbbuf)
{
	...
	age_out = (face_age_out_t*)post_process_face_age(outdata);
	if (age_out == NULL)
	{
		printf("aml_module_age_output_get error\n");
		return -1;
	}
	printf("The age is : %d\n",age_out->age);
	return ret;
...
void* init_network(int argc,char **argv)
{
	...
	printf("the input type should be 64*64*3\n");
	input_width = 64;
	input_high = 64;
